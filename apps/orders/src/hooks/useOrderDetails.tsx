...
export const orderIncludeAttribute = [
  ...
  // `payment_links` is deliberately absent: an include the token cannot read
  // fails the whole order request, and the ability for it is newer than some
  // environments. `useOrderPaymentLinks` fetches them on their own, where a
  // refusal costs only the list. Move it here once the grant is everywhere.
  "payment_sessions",
