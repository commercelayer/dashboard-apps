...
/**
 * Legacy is narrower than "not new": an order that carries no payment at all,
 * a free one for instance, has neither a session nor a legacy artifact, and
 * reading it as legacy would apply rules the 2026-05 model has dropped. Such
 * an order is left to core, which is the only side that knows which model the
 * organization is on.
 */
...
/** The STI type of a payment setting, e.g. `payment_setting_stripes`. */
type PaymentSettingType = PaymentSetting["type"]

/**
 * Payment settings whose gateway can produce a hosted payment link. Core
 * refuses every other one with a 422 (`LinkableValidator`, mirroring the
 * `linkable?` predicate on the setting), but does not expose that predicate as
 * a readable field, so the list is repeated here. The moment it becomes an
 * attribute, this goes.
 */
const LINKABLE_SETTING_TYPES = [
  "payment_setting_adyens",
  "payment_setting_checkout_coms",
  "payment_setting_stripes",
] as const satisfies ReadonlyArray<PaymentSettingType>

/**
 * Of those, the ones whose link request is built from a session token, so the
 * session has to exist before the link (`require_payment_session?` in core).
 * Stripe is the other way round: its session is created from the checkout
 * session once the customer has paid.
 */
const SESSION_FIRST_SETTING_TYPES = [
  "payment_setting_adyens",
  "payment_setting_checkout_coms",
] as const satisfies ReadonlyArray<PaymentSettingType>

/** Payment settings this order can be asked to pay through a link. */
export function getLinkablePaymentSettings(order: Order): PaymentSetting[] {
  const linkable: readonly PaymentSettingType[] = LINKABLE_SETTING_TYPES

  return (order.available_payment_settings ?? []).filter((setting) =>
    linkable.includes(setting.type),
  )
}

/** True when the link cannot be created before its payment session. */
export function requiresPaymentSession(setting: PaymentSetting): boolean {
  const sessionFirst: readonly PaymentSettingType[] =
    SESSION_FIRST_SETTING_TYPES

  return sessionFirst.includes(setting.type)
}

/**
 * Fields core added to `payment_links` after the SDK build this app is pinned
 * to (`amount_cents` and `name`, plus the order's `payment_links` itself).
 * Delete once the pin catches up.
 */
export type PaymentLinkWithAmount = PaymentLink & {
...
/**
 * A link's own `payment_setting` comes back as the base `payment_settings`
 * type, which names no gateway and so resolves to no logo. The order knows
 * better: both the settings it offers and the settings its sessions used carry
 * the STI type (`payment_setting_adyens` and friends), and the link references
 * one of them by id.
 */
export function getPaymentLinkSetting(
  ...
  return known.find((setting) => setting.id === settingId)
}

/**
 * The links worth showing next to the sessions, newest first.
 *
 * `completed` ones are left out: a completed link has become a payment session,
 * which the list already shows, and repeating it would count the same money
 * twice on screen.
 */
export function getOutstandingPaymentLinks(
  links: PaymentLinkWithAmount[],
